document.addEventListener("DOMContentLoaded", function () {
    ...
    function setupDevBlocks() {
        ...
        // Nettoie les anciennes clés qui ne correspondent plus au plan actuel
        const validTitles = new Set(sections.map((section) => section.title));
        Object.keys(content.dev).forEach((key) => {
            if (!validTitles.has(key)) delete content.dev[key];
        });

        // Protection UX : évite de créer des centaines de blocs qui figent l'interface
        let displayedSections = sections;
        if (sections.length > MAX_DEV_SECTIONS) {
            displayedSections = sections.slice(0, MAX_DEV_SECTIONS);
            const warning = document.createElement("p");
            warning.style.cssText = "color:#b45309; background:#fffbeb; border:1px solid #fde68a; border-radius:8px; padding:10px; margin:0 0 10px 0;";
            warning.textContent = `Plan très long détecté (${sections.length} sections). Affichage limité aux ${MAX_DEV_SECTIONS} premières sections pour garder l'application fluide.`;
    ...
    function updatePreview() {
        ...
        const onBreak = () => newPage();

        // 1. Rendu du Sommaire (Plan)
        if (content.plan) {
            forceSectionNewPage();
            currentPageContent = renderSection("SOMMAIRE", content.plan, currentPageContent, onBreak);
        }

        // 2. Rendu de l'Introduction
        if (content.intro) {
            forceSectionNewPage();
            currentPageContent = renderSection("INTRODUCTION", content.intro, currentPageContent, onBreak);
        }
        ...
        if (hasDev) {
            ...
            orderedDevTitles.forEach((sectionTitle) => {
                ...
                currentPageContent = renderSection(sectionTitle, content.dev[sectionTitle], currentPageContent, onBreak);
            });
        }
        ...
        if (content.conclu) {
            forceSectionNewPage();
            currentPageContent = renderSection("CONCLUSION", content.conclu, currentPageContent, onBreak);
        }

        // Toujours afficher une page vide si aucun contenu
        if (pageNum === 0) {
            newPage();
        }
